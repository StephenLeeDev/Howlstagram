    ...
    inner class AlarmRecyclerviewAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            when(alarmDTOList[position].kind) {
                0 -> {
                    val content = alarmDTOList[position].userId + getString(R.string.alarm_favorite)
                    view.commentviewitem_textview_profile.text = content
                }
                1 -> {
                    val content = alarmDTOList[position].userId + " " + getString(R.string.alarm_comment) + " of " + alarmDTOList[position].message
                    view.commentviewitem_textview_profile.text = content
                }
                2 -> {
                    val content = alarmDTOList[position].userId + " " + getString(R.string.alarm_follow)
                    view.commentviewitem_textview_profile.text = content
                }
            }
        }
